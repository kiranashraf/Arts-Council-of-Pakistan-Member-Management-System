...
namespace projectdatabse
{
    public partial class Form10 : Form
    {
        ...
        public Form10(Form1 mainForm, int year)
        {
            this.mainForm = mainForm;
            this.year = year;
            InitializeComponent();
            //test();
            head1.Text = "Member Voter List " + year;
            head2.Text = "Member Voter List " + year;
            label29.Text = "Voting Members " + year;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            to.ReadOnly = false;
            receiptno.ReadOnly = false;
            amt.ReadOnly = false;
            recpt.ReadOnly = false;
            if (my_Param != null)
            {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (this.mem_no.Text != "-")
            {
                ...
                command.CommandText = @"INSERT INTO memsub"+year+"voter (Memno ,Recept,Todate,AMT,RECPTNO)"
                + "VALUES"
                + "(@Memno ,@receipt, @to, @amt, @rcptno)";
               ...
               if(receiptno.Text!="")
                command.Parameters.AddWithValue("@receipt", Convert.ToInt32(receiptno.Text));
               else
                   command.Parameters.AddWithValue("@receipt", null);
                command.Parameters.AddWithValue("@to", to.Text);
                if (amt.Text != "")
                command.Parameters.AddWithValue("@amt", Convert.ToInt32(amt.Text));
                else
               command.Parameters.AddWithValue("@amt", null);
                if (recpt.Text != "")
                command.Parameters.AddWithValue("@rcptno", Convert.ToInt32(recpt.Text));
                else
                 command.Parameters.AddWithValue("@rcptno", null);
        ...
        public void test()
        {
            ...
            if (tabControl1.SelectedTab == tabPage2)
            {
                ...
                string query = String.Format("SELECT * FROM memsub"+year+"voter"); 
            ...
            else if (tabControl1.SelectedTab == tabPage1)
            {
                ...
                string query = String.Format("SELECT cooperative_form.ID,cooperative_form.Memno, all_members.Mr_Mrs, all_members.Name,cooperative_form.Recept,cooperative_form.RECPTNO,cooperative_form.AMT,cooperative_form.Todate,all_members.Father_Husband, all_members.Receipt, all_members.Add1, all_members.Add2, all_members.Add3, all_members.Add4, all_members.Town, all_members.Area, all_members.Phone1, all_members.Phone2, all_members.Mobile, all_members.EMail, all_members.Profession_MK, all_members.dob , all_members.Profession, all_members.Qualification, all_members.Proposed_by, all_members.Seconder, all_members.nic, all_members.Category,all_members.foi1,all_members.foi2,all_members.foi3,all_members.image FROM " + "memsub" + year + "voter AS {0} LEFT JOIN {1} ON {2} = {3}", "cooperative_form", "all_members", "cooperative_form.Memno", "all_members.Memno");
                try
                {
                    conn.Open();
                    MySqlCommand command = new MySqlCommand(query, conn);
                    ...
                    rec1.DataBindings.Add(new Binding("Text", bindingSource2, "Recept", true));
                    amount.DataBindings.Add(new Binding("Text", bindingSource2, "AMT", true));
                    rec2.DataBindings.Add(new Binding("Text", bindingSource2, "RECPTNO", true));
                    todate.DataBindings.Add(new Binding("Text", bindingSource2, "Todate", true));
        ...
        public void subtest()
        {
            ...
            rec1.DataBindings.Clear();
            rec2.DataBindings.Clear();
            amount.DataBindings.Clear();
            todate.DataBindings.Clear();
        ...
        {
            filter.Text = amountToolStripMenuItem.Text;
            filtertext = "AMT";
        }
        ...
        {
            filter1.Text = toolStripMenuItem1.Text;
            filtertext = "Memno";
        }

        private void toolStripMenuItem2_Click(object sender, EventArgs e)
        {
            filter1.Text = toolStripMenuItem2.Text;
            filtertext = "RECPTNO";
        }

        private void toolStripMenuItem3_Click(object sender, EventArgs e)
        {
            filter1.Text = toolStripMenuItem3.Text;
            filtertext = "TO";
        }

        private void toolStripMenuItem4_Click(object sender, EventArgs e)
        {
            filter1.Text = toolStripMenuItem4.Text;
            filtertext = "AMT";
        }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection conn = new MySqlConnection(conString);
            MySqlCommand command = conn.CreateCommand();
            command.CommandText = @"UPDATE " + "memsub" + year + "voter SET Memno = @Memno, Recept = @receipt, AMT = @amt, Todate = @to, RECPTNO= @rcptno  WHERE ID=@ID";
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection conn = new MySqlConnection(conString);
            MySqlCommand command = conn.CreateCommand();
            command.CommandText = @"DELETE from " + "memsub" + year + "voter WHERE ID=@ID";
            command.Parameters.AddWithValue("@ID", memid.Text.ToString());
            try
            {
                conn.Open();

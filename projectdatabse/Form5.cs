    ...
    public partial class Form5 : Form
    {
            ...
            if (my_Param != null)
            {
                ...
                try
                {
                    ...
                    if (myDataReader.HasRows)
                    {
                        ...
                        //if (!string.IsNullOrEmpty(myDataReader.GetString("Mr_Mrs").ToString()))
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (this.mem_no.Text != "-")
            {
                ...
                command.CommandText = @"INSERT INTO  expiredmem (Field1,Memno ,Mr_Mrs ,Name ,Father_Husband ,Receipt ,dob ,Add1 ,Add2 ,Add3 ,Add4 ,Town ,Area ,Phone1 ,Phone2 ,Mobile ,EMail ,Profession_MK ,Profession ,Qualification ,Proposed_by ,Seconder ,nic ,Category,foi1,foi2,foi3,image)"
                + "VALUES"
                ...
                command.Parameters.AddWithValue("@Receipt", receipt.Text);
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            MySqlConnection conn = new MySqlConnection(conString);
            string query = String.Format("SELECT `ID`,`Memno`, `Mr_Mrs`, `Name`, `Father_Husband`, `Receipt`, `Add1`, `Add2`, `Add3`, `Add4`, `Town`, `Area`, `Phone1`, `Phone2`, `Mobile`, `EMail`, `Profession_MK`, `dob` , `Profession`, `Qualification`, `Proposed_by`, `Seconder`, `nic`, `Category`,`foi1`,`foi2`,`foi3`,`image` FROM {0}", "expiredmem");
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            string conString = "Server=localhost;Port=3306;Database=acopak;Uid=root;password='root'";
            int n=0;
            string[] temp=new string[179];
            int[] id = new Int32[179];
            MySqlConnection conn = new MySqlConnection(conString);
            
            string query = String.Format("SELECT `ID`,`Add1`, `Add2`, `Add3`, `Add4`,`Field4`,`Field6`,`Field5` FROM {0}", "expiredmem01");
            MySql.Data.MySqlClient.MySqlDataReader myDataReader;
            try
            {
                conn.Open();
                ...
                if (myDataReader.HasRows)
                {
                    ...
                    {
                        //mid = Convert.ToInt32(myDataReader["ID"]);
                        if (!string.IsNullOrEmpty(myDataReader.GetString("Field4")))
                        {
                            
                            temp[n] = myDataReader.GetString("Add4") + " " + myDataReader.GetString("Field4");
                            id[n] = myDataReader.GetInt32("ID");
                            if (!string.IsNullOrEmpty(myDataReader.GetString("Field6")))
                            {
                                temp[n] = temp[n] + " " + myDataReader.GetString("Field6");
                                if (!string.IsNullOrEmpty(myDataReader.GetString("Field5")))
                                {
                                    temp[n] = temp[n] + " " + myDataReader.GetString("Field5");
                                }
                ...
                MySqlCommand command1 = conn.CreateCommand();
                            command1.CommandText = @"UPDATE expiredmem01 SET Add4 = @add4 WHERE ID=@ID1";
                            
                            command1.Parameters.AddWithValue("@ID1", id[k]);
                            command1.Parameters.AddWithValue("@add4", temp[k]);

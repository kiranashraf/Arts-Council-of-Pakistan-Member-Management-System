    ...
    public partial class Form8 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (this.mem_no.Text != "-")
            {
                ...
                command.CommandText = @"INSERT INTO  cooperative_form (Memno ,Form_No ,Receipt_No ,Remarks)"
                + "VALUES"
                + "(@Memno ,@Receipt ,@form  ,@remarks)";

                command.Parameters.AddWithValue("@Memno", mem_no.Text);
                command.Parameters.AddWithValue("@Receipt", receiptno.Text);
                command.Parameters.AddWithValue("@form", form.Text);
        ...
        public void test()
        {
            ...
            if (tabControl1.SelectedTab == tabPage2)
            {
                ...
                string query = String.Format("SELECT * FROM {0}", "cooperative_form");
            ...
            else if (tabControl1.SelectedTab == tabPage1)
            {
                ...
                string query = String.Format("SELECT cooperative_form.ID,cooperative_form.Memno, all_members.Mr_Mrs, all_members.Name,cooperative_form.Form_No,cooperative_form.Receipt_No,cooperative_form.Remarks,all_members.Father_Husband, all_members.Receipt, all_members.Add1, all_members.Add2, all_members.Add3, all_members.Add4, all_members.Town, all_members.Area, all_members.Phone1, all_members.Phone2, all_members.Mobile, all_members.EMail, all_members.Profession_MK, all_members.dob , all_members.Profession, all_members.Qualification, all_members.Proposed_by, all_members.Seconder, all_members.nic, all_members.Category,all_members.foi1,all_members.foi2,all_members.foi3,all_members.image FROM {0} LEFT JOIN {1} ON {2} = {3}", "cooperative_form", "all_members", "cooperative_form.Memno", "all_members.Memno");
                try
                {
                    conn.Open();
                    MySqlCommand command = new MySqlCommand(query, conn);
                    ...
                    formno.DataBindings.Add(new Binding("Text", bindingSource2, "Form_No", true));
                    recno.DataBindings.Add(new Binding("Text", bindingSource2, "Receipt_No", true));
                    remark.DataBindings.Add(new Binding("Text", bindingSource2, "Remarks", true));
        ...
        private void img_TextChanged(object sender, EventArgs e)
        {
            if (img.Text != "")
            {
                ...
                pic1.ImageLocation = AppDomain.CurrentDomain.BaseDirectory + "memberpics/no_icon.gif";
        }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection conn = new MySqlConnection(conString);
            MySqlCommand command = conn.CreateCommand();
            command.CommandText = @"UPDATE cooperative_form SET Memno = @Memno, Form_No=@form, Receipt_No=@receipt,Remarks=@remark WHERE ID=@ID";
            ...
            command.Parameters.AddWithValue("@receipt", recno.Text.ToString());
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection conn = new MySqlConnection(conString);
            MySqlCommand command = conn.CreateCommand();
            command.CommandText = @"DELETE from cooperative_form  WHERE ID=@ID";
            command.Parameters.AddWithValue("@ID", memid.Text.ToString());
            try
            {
                conn.Open();
